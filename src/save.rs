...
pub fn save_layout() {
    ...
    if status.success() {
        ...
        eprintln!("Failed to save layout with exit code: {}", status.code().unwrap_or(-1));
        return;
    }
    ...
    for pid in pids {
        if let Some(pid_number) = extract(pid,&pid_re) {
            ...
            if let Err(e) = save_cmdline_args(pid_number,&pid_output) {
                eprintln!("Failed to save command line args for PID {}: {}", pid_number, e);
            }
        }
    }
    ...
    for  workspace in workspaces {
        if let Some(workspace_number) = extract(workspace,&layout_re) {
            ...
            if let Err(e) = save_workspace_info(workspace_number,&layout_output) {
                eprintln!("Failed to save command line args for PID {}: {}", workspace_number, e);
            }
        }
    }
}
...
pub fn extract(line: &str,re:&Regex) -> Option<u32> {
    re.captures(line).and_then(|caps| {
        caps.get(1).map(|m| m.as_str().parse::<u32>().unwrap_or(0))
    })
}
